...
\begin{enumerate}
\item Create a new branch when you create a new task 
(\href{https://github.com/gslab-econ/ra-manual/wiki/Tasks}{G\&S}: ``The assignee should create a separate git branch for each task that involves code or output stored in the repository.'')
Either (1) create it on the remote repo using the GitHub web interface and then fetch and check it out or (2) create it locally and then push it to the remote repo.
...
\item Referencing \texttt{task\_flow\_branchdiff.png},
    ...
    it would be costly to delete the output folder of \texttt{download\_data} every time,
...
scripts in \texttt{tasks/actions\_scripts}.
