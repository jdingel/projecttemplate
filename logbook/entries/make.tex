...
\item Zachary M. Jones: \href{https://web.archive.org/web/20240222042605/http://zmjones.com/make/}{GNU Make for Reproducible Data Analysis}
\end{itemize}
...
\subsection{Validating Makefiles}

A semi-common mistake is to submit a Makefile that only worked on your machine because of the state of the files.
For example, an input existed on your machine, so you forgot to supply a rule to produce that input,
which others will need.
There is a shell script in the task \texttt{actions\_scripts} that executes \texttt{make -n} on every Makefile in the \texttt{tasks} folder to validate them.
This script is run automatically as a GitHub action when a pull request is submitted for review.
The script accepts a list of tasks as its argument if you want to only validate a subset of tasks.
